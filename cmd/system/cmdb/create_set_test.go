...
var _ = Describe("cmdb create_set", func() {
	var tmpDir string

	BeforeEach(func() {
		var err error
		tmpDir, err = os.MkdirTemp("", "bk-cli-cmdb-create-set-*")
	...
	It("synthesizes defaults from required flags", func() {
		Expect(systemtest.SetupTestContext("https://bkapi.example.com/api")).To(Succeed())

		cmd := newCreateSetCmd(systemtest.BuildDeps(true))
		...
		Expect(request["url"]).To(Equal(
			"https://bkapi.example.com/api/bk-cmdb/prod/api/v3/open/set/2",
		))

		body := request["body"].(map[string]any)
		Expect(body).To(Equal(map[string]any{
			"bk_parent_id":    float64(2),
			"bk_set_name":     "web",
			"set_template_id": float64(0),
			"default":         float64(0),
		}))
	})
})
