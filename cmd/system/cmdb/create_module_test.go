...
var _ = Describe("cmdb create_module", func() {
	var tmpDir string

	BeforeEach(func() {
		var err error
		tmpDir, err = os.MkdirTemp("", "bk-cli-cmdb-create-module-*")
	...
	It("synthesizes defaults from required flags", func() {
		...
		cmd := newCreateModuleCmd(systemtest.BuildDeps(true))
		Expect(cmd.Flags().Set("bk_biz_id", "2")).To(Succeed())
		Expect(cmd.Flags().Set("bk_set_id", "10")).To(Succeed())
		Expect(cmd.Flags().Set("bk_module_name", "web")).To(Succeed())
		...
		Expect(request["url"]).To(Equal(
			"https://bkapi.example.com/api/bk-cmdb/prod/api/v3/open/module/2/10",
		))

		body := request["body"].(map[string]any)
		Expect(body).To(Equal(map[string]any{
			"bk_parent_id":   float64(10),
			"bk_module_name": "web",
		}))
	})
})
