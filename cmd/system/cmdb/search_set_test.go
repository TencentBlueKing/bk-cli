...
var _ = Describe("cmdb search_set", func() {
	var tmpDir string

	BeforeEach(func() {
		var err error
		tmpDir, err = os.MkdirTemp("", "bk-cli-cmdb-search-set-*")
	...
	It("synthesizes the request body from flags", func() {
		...
		cmd := newSearchSetCmd(systemtest.BuildDeps(true))
		Expect(cmd.Flags().Set("bk_biz_id", "2")).To(Succeed())
		Expect(cmd.Flags().Set("bk_set_name", "web")).To(Succeed())
		...
		Expect(request["url"]).To(Equal(
			"https://bkapi.example.com/api/bk-cmdb/prod/api/v3/open/set/search/0/2",
		))

		body := request["body"].(map[string]any)
		Expect(body["fields"]).To(Equal([]any{"bk_set_id", "bk_set_name"}))
		Expect(body["condition"]).To(Equal(map[string]any{"bk_set_name": "web"}))
