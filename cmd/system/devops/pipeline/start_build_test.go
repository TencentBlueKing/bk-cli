...
var _ = Describe("pipeline start_build", func() {
	var tmpDir string

	BeforeEach(func() {
		var err error
		tmpDir, err = os.MkdirTemp("", "bk-cli-devops-pipeline-start-build-*")
	...
	It("escapes dynamic path and query values in the outgoing request URL", func() {
		Expect(systemtest.SetupTestContext("https://bkapi.example.com/api")).To(Succeed())

		cmd := newStartBuildCmd(systemtest.BuildDeps(true))
		...
		Expect(request["url"]).To(Equal(
			"https://bkapi.example.com/api/devops/prod/v4/apigw-user/projects/team%2Fservice/build_start?pipelineId=pipe%3Fx%3D1%26y%3D2",
		))
	})
})
